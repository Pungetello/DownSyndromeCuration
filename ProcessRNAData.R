...
process_data = function(srr, index, annotation, mac){
  ...
  write_tsv(counts_df, file=paste0(getwd(), "/Data/NormalizedData/", srr, mac, "_gene_counts_reordered.tsv"))
...
calculate_data_files = function(feature_counts, mac){
  ...
  write_tsv(rpkm_df, file=paste0(getwd(), "/Data/NormalizedData/", srr, mac, "_RPKM_reordered.tsv"))
  ...
  write_tsv(tpm_df, file=paste0(getwd(), "/Data/NormalizedData/", srr, mac, "_TPM_reordered.tsv"))
}
...
combine_results_per_GSE = function(){
  ...
  for(gse in gses){
    ...
    combine_files(gse, srrs, paste0(mac, "_gene_counts_reordered.tsv"))
    combine_files(gse, srrs, paste0(mac, "_TPM_reordered.tsv"))
    combine_files(gse, srrs, paste0(mac, "_RPKM_reordered.tsv"))
...
for (srr in srrs){
  ...
  if(Datasets$Organism[Datasets$Name == geo_id] == "human"){
    ...
    MAC = "_MAC"
    #mouse with MAC
    ref = paste0(getwd(), "/RefGenomes/mouse_plus_mac_reordered.fa")
  ...
  if (file.exists(paste0(getwd(), "/Data/NormalizedData/", srr, MAC, "_RPKM_reordered.tsv"))){
